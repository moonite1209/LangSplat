...
class GaussianSplattingGUI:
    ...
    def load_model(self):
        ...
        checkpoint = os.path.join(self.opt.MODEL_PATH, 'chkpnt30000.pth')
        (model_params, first_iter) = torch.load(checkpoint)
        self.engine['scene'].restore(model_params, args, mode='test')
        # self.engine['scene'].get_language_feature[self.engine['scene'].get_language_feature.isnan()] = 0 #TODO
        self.raw_language_feature = self.codec.decode(self.engine['scene'].get_language_feature).detach()
    ...
    def do_pca(self):
        sems = self.engine['scene'].get_language_feature.detach().clone()
        N, C = sems.shape
        torch.manual_seed(0)
    ...
    def fetch_data(self, view_camera):
        ...
        scene_outputs = render(view_camera, self.engine['scene'], self.opt, self.bg_color, self.opt)
        # --- RGB image --- #
        ...
        # --- semantic image --- #
        sems = scene_outputs['language_feature_image'].permute(1, 2, 0).cpu()
        H, W, C = sems.shape
        sems /= (torch.norm(sems, dim=-1, keepdim=True) + 1e-6)
